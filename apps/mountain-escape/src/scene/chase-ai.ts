...
export class ChaseAI {
  ...
  update(input: ChaseInput): ChaseSnapshot {
    ...
    const lookPenalty = this.isLookingAtChaser(input.playerYaw)
      ...
      : 1;
    const speed = clamp(
      (this.config.BASE_CHASE_SPEED + this.config.CHASE_SPEED_RAMP * input.elapsed) *
        lookPenalty,
      0,
  ...
  private isLookingAtChaser(playerYaw: number): boolean {
    ...
    const angle = Math.abs(
      Math.atan2(Math.sin(playerYaw - chaserDirection), Math.cos(playerYaw - chaserDirection)),
    );
